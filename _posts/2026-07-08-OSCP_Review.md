...
An example: for Windows privilege escalations, I liked having mimikatz, winpeas, powerup, powerview... (We're talking about CTF environments here, not real-world ones). So, I would create a tools/windows folder with all these tools, and in my notes, I had a PowerShell one-liner like this one that would download everything into the Public user folder. That way, I could immediately start working exactly the way I like to.
